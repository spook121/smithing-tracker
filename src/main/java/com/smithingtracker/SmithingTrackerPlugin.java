...
public class SmithingTrackerPlugin extends Plugin
{
	...
	{
		session.reset(); barsUsed.clear(); barsInBank.clear(); itemsSmithed.clear(); cachedItemTotals.clear();
	...
	@Subscribe public void onConfigChanged(ConfigChanged e)
	{
		...
		if (key.endsWith("All"))
		{
			...
			if (lower.contains("bronze")) metalCap = "Bronze";
			else if (lower.contains("iron")) metalCap = "Iron";
			else if (lower.contains("steel")) metalCap = "Steel";
			else if (lower.contains("mithril")) metalCap = "Mithril";
			else if (lower.contains("adamant")) metalCap = "Adamant";
			else if (lower.contains("rune")) metalCap = "Rune";
			else return;

			boolean enabled = Boolean.parseBoolean(e.getNewValue());
			String[] suffixes = {
					"Dagger","Axe","Mace","MedHelm","Bolts","Sword","Nails","DartTips",
					"Scimitar","Spear","Hasta","ArrowTips","Limbs","Longsword","JavelinTips",
					"FullHelm","Knives","SqShield","Warhammer","Battleaxe","Chainbody",
					"Kiteshield","Claws","TwoHandedSword","Platelegs","Plateskirt","Platebody","Cannonball"
			};

			for (String suffix : suffixes)
			{
				String itemKey = "show" + metalCap + suffix;
				configManager.setConfiguration("smithingtracker", itemKey, enabled);
			}
		}
	}
	...
	@Subscribe public void onItemContainerChanged(ItemContainerChanged e)
	{
		...
		else if (e.getContainerId() == InventoryID.INVENTORY.getId()) updateCachedTotalsFromInventory();
	}
	...
	private void scanForAllSmithableItems()
	{
		...
		Set<Integer> found = new HashSet<>(); String[] pre = {"bronze", "iron", "steel", "mithril", "adamant", "rune"}; // cannonballs handled separately
		for (int i = 0; i < 40000; i++) { try { String n = itemManager.getItemComposition(i).getName(); if (n == null) continue; String l = n.toLowerCase(); boolean ok = false; for (String p : pre) if (l.startsWith(p + " ")) { ok = true; break; } if (l.contains("cannonball")) ok = true; if (!ok) continue; if (isSmithableType(l)) { int tot = (bank!= null? bank.count(i) : 0) + (inv!= null? inv.count(i) : 0); if (tot > 0) { found.add(i); cachedItemTotals.put(i, tot); } } } catch (Exception ignored) {} }
		itemsSmithed.clear(); for (Integer id : found) itemsSmithed.put(id, 1);
	}

	private boolean isSmithableType(String n) { if (n.contains("pickaxe")) return false; return n.contains("2h sword") || n.contains("platebody") || n.contains("platelegs") || n.contains("plateskirt") || n.contains("full helm") || n.contains("med helm") || n.contains("chainbody") || n.contains("sq shield") || n.contains("kiteshield") || n.contains("scimitar") || n.contains("longsword") || n.contains("sword") || n.contains("dagger") || n.contains("axe") || n.contains("mace") || n.contains("warhammer") || n.contains("battleaxe") || n.contains("claws") || n.contains("spear") || n.contains("hasta") || n.contains("javelin") || n.contains("dart tip") || n.contains("arrowtip") || n.contains("knives") || n.contains("bolts") || n.contains("limbs") || n.contains("nails") || n.contains("cannonball"); }
	...
	public String getItemKeyFromName(String lower)
	{
		...
		if (l.contains("cannonball")) return "Cannonball";
	...
	public boolean shouldShowItem(int id) {
		try {
			String name = itemManager.getItemComposition(id).getName();
			if (name == null) return true;
			String lower = name.toLowerCase();

			// Cannonballs are special - made from steel at furnace with ammo mould
			if (lower.contains("cannonball"))
			{
				return config.showSteelCannonball() && config.showCannonballs();
			}

			String metal;
			if (lower.startsWith("bronze ")) metal = "bronze";
			else if (lower.startsWith("iron ")) metal = "iron";
			else if (lower.startsWith("steel ")) metal = "steel";
			else if (lower.startsWith("mithril ")) metal = "mithril";
			else if (lower.startsWith("adamant ")) metal = "adamant";
			else if (lower.startsWith("rune ")) metal = "rune";
			else return true;
			...
			switch (metal) {
				...
				case "steel":
					switch (key) {
						...
						case "Cannonball": return config.showSteelCannonball() && config.showCannonballs();
						default: return true;
					}
				case "mithril":
					switch (key) {
